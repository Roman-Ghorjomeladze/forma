...
`dist/` is committed and prebuilt, so you can deploy without building at all.

```
public/        static shell: index.html, styles.css, manifest, icons, vendored React
src/           TypeScript sources (compiled by tsc to dist/app)
...
dist/          the deployable app (generated by `npm run build`)
```
...
```bash
cd my-app
npm run serve          # serves dist/ and prints a http://192.168.x.x:5173 address
```
...
Any static host works — GitHub Pages, Netlify, Vercel, Cloudflare Pages. Upload the **contents of `dist/`**.

GitHub Pages, for example:

1. Create a repo, push this folder.
2. Settings → Pages → *Deploy from a branch* → branch `main`, folder `/dist`  
   (or copy `dist/` into a `docs/` folder and choose `/docs`).
...
npm run dev            # tsc --watch + copies public/ + serves dist/ on the network with live rebuilds
npm run build          # production build into dist/ (also regenerates sw.js with a fresh cache version)
npm run typecheck
```
