import type { Dish, Ingredient, MealCategory } from '../lib/models.js';
...
export const PER100: Record<string, [number, number, number, number]> = {
...
export const UNIT_G: Record<string, number> = { tbsp: 14, tsp: 5, slice: 40, pcs: 1, scoop: 30, ml: 1, g: 1, cup: 240, clove: 4, wrap: 60, half: 70 };
export const PIECE_G: Record<string, number> = { Egg: 50, Banana: 120, Apple: 180, 'Tortilla wrap': 60, Avocado: 140, Onion: 110, 'Bell pepper': 120, Zucchini: 200, Carrot: 70, Garlic: 4, Tomato: 120, Lemon: 60 };

let n = 0;
function ing(name: string, amount: number, unit: string): Ingredient {
  const per = PER100[name];
  let grams = amount;
  if (unit === 'pcs') grams = amount * (PIECE_G[name] ?? 100);
  else if (unit === 'clove') grams = amount * 4;
  else if (unit === 'half') grams = amount * 70;
...
interface Seed {
  id: string; name: string; category: MealCategory; servings: number; prepMin: number; cookMin: number; tags: string[];
  ingredients: Ingredient[]; steps: string[]; notes?: string;
}
...
export function seedDishes(now = Date.now()): Dish[] {
