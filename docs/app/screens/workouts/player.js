...
export function PlayerScreen({ id }) {
        ...
        const tick = () => {
            ...
            if (secLeft <= prefs.countdownSeconds && secLeft >= 1 && tm.cuedSecond !== secLeft && durationOf(tm.index) > prefs.countdownSeconds) {
                ...
                if (soundOn)
                    sounds.tick();
                if (tm.index >= 0 || true)
                    speak(t(`player.count.${secLeft}`), { rate: 1.2 });
            }
    ...
    const quit = async () => {
        if (phase === 'ready' || phase === 'done') {
            navigate(`/forma/workouts/${id}`, { replace: true });
            return;
        }
    ...
    if (workout === null || steps.length === 0)
        return _jsx("div", { className: "player", children: _jsx(Empty, { title: t('player.nothingToPlay'), text: t('player.noExercisesYet'), action: _jsx(Button, { variant: "secondary", onClick: () => navigate('/forma/workouts'), children: t('common.back') }) }) });
    const tm = t2.current;
    const idx = tm.index;
