...
export const flagUrl = (c) => `./flags/${c}.svg`;
export function countryName(c, lang) { return lang === 'ka' ? c.nk : c.n; }
export function capitalName(c, lang) { return lang === 'ka' ? c.capk : c.cap; }
export function continentName(r) { return r === 'all' ? tGlobal('flags.all') : tGlobal(`flags.continent.${r}`); }
export function continentShort(r) { return r === 'all' ? tGlobal('flags.all') : tGlobal(`flags.continentShort.${r}`); }
export const byCode = new Map(COUNTRIES.map((c) => [c.c, c]));
export function filterCountries(filter, lang) {
...
export function searchCountries(list, q, lang) {
...
export function buildQuiz(mode, continent, history, length = QUIZ_LENGTH) {
    ...
    return asked.map((answer) => {
        ...
        const ok = (c) => c.c !== answer.c && (mode !== 'capital' || c.cap !== answer.cap);
...
export function newResult(mode, continent, score, total, missed) {
    return { id: uid('qz'), mode, continent, score, total, missed, playedAt: Date.now() };
}
// ---- stats ---------------------------------------------------------------------------------------
export function bestScore(history, mode, continent) {
    let best = null;
    for (const r of history)
        if (r.mode === mode && r.continent === continent && (!best || r.score / r.total > best.score / best.total))
            best = r;
    return best;
}
...
export function perfectRounds(history) { return history.filter((r) => r.score === r.total).length; }
/** Countries the user has got wrong in recent rounds (any mode), most recent first, unique. */
export function weakSpots(history, limit = 12) {
    const seen = new Set();
    const out = [];
    for (const r of history)
        for (const code of r.missed) {
            if (!seen.has(code)) {
                seen.add(code);
...
export function resultTitleKey(score, total) {
    const p = score / total;
    if (p === 1)
        return 'flags.result.perfect';
    if (p >= 0.8)
        return 'flags.result.great';
    if (p >= 0.5)
        return 'flags.result.ok';
    return 'flags.result.keepGoing';
}
export const CONTINENT_LIST = ['Europe', 'Asia', 'Africa', 'North America', 'South America', 'Oceania'];
