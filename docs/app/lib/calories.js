...
export function blockSeconds(block, ex) {
    if (block.type === 'rest')
        return block.seconds;
    if (block.type === 'exercise') {
        const e = ex.get(block.exerciseId);
        if (block.seconds != null)
            return block.seconds;
        if (block.reps != null)
            return Math.round(block.reps * (e?.secPerRep ?? 3));
...
export function estimateWorkout(workout, ex, weightKg) {
...
export function suggestDailyKcal(weightKg, heightCm, age, sex, goal = 'maintain') {
