...
export function categoryName(c) {
    ...
    if (key && c.name === DEFAULT_CATEGORIES.find((d) => d.id === c.id)?.name)
        return tGlobal(key);
    return c.name;
}
...
export async function ensureOtherCategory() {
    const all = await getAll('categories');
    const found = all.find((c) => c.id === OTHER_CATEGORY_ID);
    if (found)
        return found;
    const c = { ...DEFAULT_CATEGORIES[DEFAULT_CATEGORIES.length - 1], order: all.length, createdAt: Date.now() };
...
export function newExpense(projectId, categoryId, partial = {}) {
    const now = Date.now();
...
export function fmtLari(n) { return `₾${fmtMoney(n)}`; }
// ---- totals ---------------------------------------------------------------------------------
export function sum(rows) { return rows.reduce((a, e) => a + e.amount, 0); }
...
export function busiestMonth(rows) {
...
export function expensesToCsv(rows, categories, project) {
...
export async function shareOrDownloadText(name, text, type = 'text/csv') {
    const nav = navigator;
    try {
