...
const stand = { head: [60, 22], torso: 'M60 32 L60 70', arms: 'M60 42 L52 64 L50 82 M60 42 L68 64 L70 82', legs: 'M60 70 L52 88 L52 105 M60 70 L68 88 L68 105' };
export const DEMOS = {
    'jumping-jack': {
        dur: 1.1, floor: true,
        poses: [
...
function animate(attr, values, dur) {
...
export function demoMarkup(key, animated = true) {
    const d = DEMOS[key] ?? DEMOS.squat;
    const p = d.poses;
    const first = p[0];
    const anim = (attr, vals) => (animated && p.length > 1 ? animate(attr, vals, d.dur) : '');
    return [
...
export function Demo({ demoKey, size = 120, animated = true, className }) {
    return (_jsx("svg", { className: className, viewBox: "-2 -2 124 112", width: size, height: size, fill: "none", stroke: "currentColor", strokeWidth: "4.5", strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true", dangerouslySetInnerHTML: { __html: demoMarkup(demoKey, animated) } }));
}
